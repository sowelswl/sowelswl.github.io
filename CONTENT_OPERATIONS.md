...
4. Create a 1200×630 social card in `/images/social/`; retain the editable SVG and publish the matching PNG in Open Graph, Twitter, and Article metadata.
5. Replace the less relevant homepage preview if the new article better explains current work; keep only two featured entries.
6. Add the canonical route to `sitemap.xml` and use an article-specific campaign name on any live-report link.
7. Verify the article at desktop and narrow widths before publishing.
