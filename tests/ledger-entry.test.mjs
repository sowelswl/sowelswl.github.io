...
test("the personal site hosts the public ledger without a project Pages deployment", async () => {
  ...
  assert.match(ledger, /全历史、近 3 个月和近 1 个月/)
  assert.match(ledger, /id="evaluation-windows"/)
  assert.match(ledger, /assets\/ledger\.css\?v=20260823-windows/)
  assert.match(ledger, /assets\/ledger\.js\?v=20260823-windows/)
  assert.match(ledger, /最近 20 个交易日/)
  assert.match(ledger, /下一交易日五个宽基实际涨跌幅/)
  ...
  assert.match(script, /directionMatches/)
  assert.match(script, /record\.outcome\?\.benchmarks/)
